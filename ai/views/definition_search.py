...
from ai.serializers.definition_search import DefinitionSearchInputSerializer
from ai.serializers.server_text_response import ServerTextResponseSerializer
from ai.utils import async_read_file, clean_llm_output, definition_query, stringify_vdb_results, unify_vdb_results
...
@router.post("/definition_search", tags=[APITags.AI], url_name="definition_search")
async def definition_search(request: HttpRequest, payload: DefinitionSearchInputSerializer = Form(...)):
    """
    API endpoint for searching for definitions of selected text using RAG (Retrieval-Augmented Generation).

    Combines vector database search with LLM completions to provide context-aware definitions.
    The workflow: validate input -> search vector DB -> load prompts -> call LLM -> render HTML response.

    Process a selected text and return an LLM-generated definition with context.

    Workflow:
        1. Validate request payload (selected_text, verses_text, book, chapter and collection_name)
    ...
    """
    file_directory = "definition_search"
    ...
    try:
        unified_results = await unify_vdb_results(verses_results + selected_text_results)
    ...
    # Search WordNet for relevant context
    try:
        definition_results = await definition_query(selected_text, request)
        definition_results = definition_results.strip()
    except Exception as e:
        logger.error(f"Error searching WordNet: {e}")
        return HttpResponse(f"Error searching WordNet: {e}", status=500, content_type="text/html")
    ...
    try:
        ...
        user_prompt = user_prompt.format(
            ...
            vector_context=stringified_unified_results,
            definition_context=definition_results,
        )
    except Exception as e:
